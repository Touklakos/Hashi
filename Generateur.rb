...
class Generateur
  ...
  def creeUneGrillev2(nbSommet)
    ...
    loop do
      ...
      puts "on prends le sommet " + indiceSommetChoisi.to_s + " en " + sommetChoisi.position.x.to_s + ":" + sommetChoisi.position.y.to_s
      #On choisi une direction dans laquelle partir
      ...
      lesAdds = tableauDeAdd[rand(0...tableauDeAdd.length)]
      puts "dans la direction " + lesAdds.to_s
      ...
      boolOutOfBounds = sommetChoisi.position.grille.caseSuivante(sommetChoisi.position,2*lesAdds[0],2*lesAdds[1])==nil
      puts !boolOutOfBounds
      puts (@grille.getCase(sommetChoisi.position.x + lesAdds[0], sommetChoisi.position.y  + lesAdds[1]).estVide())
      if !boolOutOfBounds && (@grille.getCase(sommetChoisi.position.x + lesAdds[0], sommetChoisi.position.y  + lesAdds[1]).estVide())
        ...
        boolSommetPlace = false
        loop do #la loop pour faire avancer
          break if boolSommetPlace
          #puts "On est en" + caseOuPlacer.x.to_s + ":" + caseOuPlacer.y.to_s
          caseUnDevant = caseOuPlacer.class == NilClass ? nil : (caseOuPlacer.grille.caseSuivante(caseOuPlacer, lesAdds[0], lesAdds[1]))
          caseDeuxDevant = caseOuPlacer.class == NilClass ? nil : (caseOuPlacer.grille.caseSuivante(caseOuPlacer, 2*lesAdds[0], 2*lesAdds[1]))
          ...
          if boolOnVaTropLoin && !(caseOuPlacer.aSommetVoisin())
            puts "on va trop loin en " + caseOuPlacer.x.to_s + ":" + caseOuPlacer.y.to_s
